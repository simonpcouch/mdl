...
mdl implements an opinionated and performant reimagining of model matrices. The package supplies one function, `mdl::mtrx()` (read: "model matrix"), that takes in a formula and data frame and outputs a numeric matrix. In some situations, it can be quite fast compared to its base R friend `model.matrix()`.
...
The factor of speedup isn't so drastic for larger datasets, and `mdl::mtrx()` can even be slower than `model.matrix()` when creating many dummy variables.
